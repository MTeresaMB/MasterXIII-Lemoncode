...
const averageNumber = (numbers: number[]): number => numbers.reduce((a: number, b: number) => a + b) / numbers.length;

const summarizeClassRoom = (studentList: Student[]): StudentSummary[] => {
  let result: StudentSummary[] = [];
  studentList.forEach((student) => {
    let summaryStudent: StudentSummary = {
      name: student.name,
      highestCalification: Math.max(...student.califications),
      averageCalification: (averageNumber(student.califications).toPrecision(3))
    };
    result.push(summaryStudent);
  });
  return result;
};
